...
def add_llama_embedding_args(parser) -> None:
    parser.add_argument(
        ...
        help="Precision used when loading the Llama encoder with the transformers backend.",
    )
    parser.add_argument(
        "--llama_device",
        type=str,
    ...
    parser.add_argument(
        "--llama_load_in_8bit",
        action="store_true",
        help="Load the transformers backend with 8-bit quantization. Requires bitsandbytes on supported hardware.",
    )
    parser.add_argument(
        "--llama_load_in_4bit",
        action="store_true",
        help="Load the transformers backend with 4-bit quantization. Requires bitsandbytes on supported hardware.",
    )
    parser.add_argument(
        ...
        help="Attention kernel for the transformers backend. 'auto' keeps the library default.",
    )
    parser.add_argument(
...
class TransformersTextEmbedder:
    def __init__(
        ...
        if AutoModel is None or AutoTokenizer is None:
            raise ImportError(
                "transformers is required for the transformers Llama backend. Install it before running this benchmark."
            )
