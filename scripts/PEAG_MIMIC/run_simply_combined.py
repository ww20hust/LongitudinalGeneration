...
from models import SimplyCombinedClassifier


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Simply combined benchmark with Llama document embeddings and lab sequence inputs.')
    ...
    parser.add_argument('--document_hidden_dim', type=int, default=256)
...
def main() -> None:
    ...
    model = SimplyCombinedClassifier(
        lab_dim=train_split.labs.shape[-1],
        document_dim=train_doc_embeddings.shape[-1],
        ...
        document_hidden_dim=args.document_hidden_dim,
    ...
    save_json(save_dir / 'metrics.json', {
        'benchmark': 'simply_combined_llama_document_plus_labs',
    ...
    print(f'Simply combined benchmark complete: {save_dir}')
